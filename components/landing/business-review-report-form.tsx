...
import { reportBusinessReview } from "@/lib/reports/actions";
...
export function BusinessReviewReportForm({ data, review }: Props) {
  const currentUserId = data.reviewForm?.userId ?? null;
  const isAuthenticated = data.reviewForm?.isAuthenticated ?? false;
  const reportReview = reportBusinessReview.bind(
    null,
    data.id,
    data.slug,
    review.id,
    ...
    return (
      <details className="mt-5 rounded-2xl border border-gray-100 bg-gray-50 p-4">
        ...
        <div className="mt-3 text-sm leading-6 text-gray-600">
          Para reportar una reseña necesitas iniciar sesión.
        </div>

        <Link
          href="/auth/login?message=Inicia+sesi%C3%B3n+para+reportar+una+rese%C3%B1a."
  ...
  return (
    <details className="mt-5 rounded-2xl border border-gray-100 bg-gray-50 p-4">
      ...
      <form action={reportReview} className="mt-4 space-y-4">
        <label className="block">
          ...
          >
            ...
            <option value="inappropriate_content">Lenguaje ofensivo o inapropiado</option>
        ...
        <label className="block">
          ...
          <textarea
            ...
            maxLength={1000}
            required
            placeholder="Explica brevemente por qué esta reseña debería revisarse."
            className="mt-2 w-full rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-950 outline-none transition focus:border-orange-400 focus:ring-4 focus:ring-orange-100"
          />
        </label>
        ...
        <p className="text-xs leading-5 text-gray-500">
          El reporte no oculta la reseña automáticamente. Un administrador la
          revisará.
        </p>
      </form>
    </details>
